...
/**
 * Bumped whenever [DataStore.buildDocument] changes how a document is laid out. It feeds the content hash, so every
 * entry written by an older version reads back as stale and gets rewritten instead of silently staying unmatchable.
 */
...
object DataStore {
    ...
    fun ingest(added: List<SearchDocument<*>>) {
        DirectoryReader.open(directory).use { reader ->
            ...
            for (doc in added) {
                val status = checkStatus(searcher, doc)
                if (status == EntryStatus.STALE || status == EntryStatus.NOT_EXISTS || status == EntryStatus.NEEDS_EMBEDDING) {
                    toEmbed.add(doc)
                }
                if (status == EntryStatus.NOT_EXISTS || status == EntryStatus.STALE) {
                    val newDoc = buildDocument(doc)
                    if (status == EntryStatus.NOT_EXISTS) {
    ...
    fun clean() {
        // Remove all elements currently not in the corpus
        val toKeep = SearchCorpus.corpus.keys.toSet()
        DirectoryReader.open(directory).use { reader ->
    ...
    fun <T> search(searcher: (IndexReader, IndexSearcher, StandardAnalyzer) -> T): T {
        DirectoryReader.open(directory).use { reader ->
